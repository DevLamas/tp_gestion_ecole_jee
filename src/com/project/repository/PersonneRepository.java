...
public class PersonneRepository {
	...
	/*
	 * Recherche d'une personne via son identifiant Parameter int id retourne objet
	 * Personne
	 */
	public Personne find(int id) {
		...
		try {
			PreparedStatement statement = this.getBdd().prepareStatement(
					"Select * from personne p INNER JOIN status s ON p.Id_status = s.Id_status where id = ?");
	...
	public ArrayList<Personne> getListEleves() {
		...
		try {
			PreparedStatement statement = this.getBdd().prepareStatement(
					"Select * from personne p INNER JOIN status s ON p.Id_status = s.Id_status where s.Id_status = 3");
	...
	public ArrayList<Personne> getListProfs() {
		...
		try {
			PreparedStatement statement = this.getBdd().prepareStatement(
					"Select * from personne p INNER JOIN status s ON p.Id_status = s.Id_status where s.Id_status = 2");
			ResultSet rs = statement.executeQuery();
			while (rs.next()) {
				Personne personne = buildObjet(rs);
				personnes.add(personne);
	...
	/*
	 * Fonction ajout d'une personne Parameter Personne personne retourne boolean
	 */
	...
	/*
	 * Fonction de modification d'une personne ciblé Parameter Personne personne
	 * retourne un boolean (true or false)
	 */
	public boolean updateEleve(Personne personne) {
		try {
			PreparedStatement statement = this.getBdd().prepareStatement(
					"update personne set nom = ?, prenom = ?, email = ?, mdp = ?, Id_status = ? where id = ?");
			statement.setString(1, personne.getNom());
			statement.setString(2, personne.getPrenom());
			statement.setString(3, personne.getEmail());
			statement.setInt(4, personne.getStatus().getId());
	...
	/*
	 * Fonction supprimant une personne sélectionner Parameter int peronneId
	...
	public Personne authentification(String email, String password) {
		try {
			PreparedStatement statement = this.getBdd().prepareStatement(
					"Select * from personne p INNER JOIN status s ON p.Id_status = s.Id_status where email = ? AND mdp = ?");
	...
	/*
	 * Fonction permettant la création des objets Personnes Parameter ResultSet
	 * result retourne un objet Personne
	 */
